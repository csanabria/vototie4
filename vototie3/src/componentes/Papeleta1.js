...
// USAR USEREDUCER const estadoInicial = 
const estadoInicial = {
...
const Papeleta1 = (props) => {
    //valores de estado local
    const [estadoLocal, dispatch] = useReducer(reducer, estadoInicial);

    //funcion que aumenta la cantidad de votos del id indicado en 1
    const Votar = id => {
        estadoLocal.Candidatos.map(c => {
            if (c.Id === id) {
                //c.CantidadVotos++;
                dispatch({type: 'votar', payload: id});
            }
        })
    }
